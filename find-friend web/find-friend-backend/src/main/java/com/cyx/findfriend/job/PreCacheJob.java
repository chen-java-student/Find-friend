package com.cyx.findfriend.job;
...
public class PreCacheJob {
   ...
   private List<Long> mainUserList= Arrays.asList(1L);
    ...
    public void doCacheRecommendUser() {
        RLock lock = redissonClient.getLock("fandfriend:precachejob:docache:lock");
        try {
            //只有一个线程能获取到锁
            if(lock.tryLock(0,30000,TimeUnit.MICROSECONDS)){
                for (Long userId : mainUserList) {
                    ...
                    String redisKey=String.format("fandfriend:user:recommend:%s",userId);
